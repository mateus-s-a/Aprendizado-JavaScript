...
const op = [
    () => {
        ...
        res.value = Number(val[0]) + Number(val[1]) // 'Number' usado para converter os valores das strings obtidas dos campos de entrada
    },

    () => {
        const val = [document.getElementById('valor1').value, document.getElementById('valor2').value].map(Number) // 'Number' usado de forma mais direta
